...
class stndmesh:
    ...
    def from_solid(self, ele, nodes, prop = None, src = None, det = None, link = None):
        ...
        prop : double NumPy array, optional
            ...
            See :func:`~nirfasterff.base.stnd_mesh.stndmesh.set_prop()` for details. 
    ...
    def from_image(self, img, param = utils.MeshingParams2D(), prop = None, src = None, det = None, link = None):
        """
        Construct mesh from a labeled 2D image using the Jonathan Shewchuk's Triangle. Calls stndmesh.from_triangle after meshing step.
    ...
    def femdata(self, freq, solver=utils.get_solver(), opt=utils.SolverOptions()):
        ...
        See :func:`~nirfasterff.forward.femdata.femdata_stnd_CW()` and :func:`~nirfasterff.forward.femdata.femdata_stnd_FD()` for details
    ...
    def femdata_tpsf(self, tmax, dt, savefield=False, beautify=True, solver=utils.get_solver(), opt=utils.SolverOptions()):
        ...
        See :func:`~nirfasterff.forward.femdata.femdata_stnd_TR()` for details
        ...
        """
        data, info = forward.femdata_stnd_TR(self, tmax, dt, savefield, beautify, solver, opt)
        return data, info
    ...
    def femdata_moments(self, max_moments=3, savefield=False, solver=utils.get_solver(), opt=utils.SolverOptions()):
        ...
        See :func:`~nirfasterff.forward.femdata.femdata_stnd_TR_moments()` for details
        ...
        data, info = forward.femdata_stnd_TR_moments(self, max_moments, savefield, solver, opt)
        return data, info
    
    def jacobian(self, freq=0, normalize=True, mus=False, solver = utils.get_solver(), opt = utils.SolverOptions()):
        """
        Calculates the Jacobian matrix
        
        Returns the Jacobian, direct field data, and the adjoint data
        
        Structure of the Jacobian is detailed in :func:`~nirfasterff.inverse.jacobian_stnd_CW()` and :func:`~nirfasterff.inverse.jacobian_stnd_FD()`
        ...
        """
        if freq>=0:
            J, data1, data2 = inverse.jacobian_stnd_FD(self, freq, normalize, mus, solver, opt)
        else:
